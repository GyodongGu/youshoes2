...
public class ImageDAO extends DAO{
	...
	public int insertProfileImage(String id, imageDetailDTO iddto) {
		int result =0;
		int result1=0;
		String sql = "insert into image values((select max(img_no)+1 from image i),'I04',?)";
		String sql1= "insert into image_detail values((select max(img_no)+1 from image_detail d),?,sysdate(),?,'Y')";
	...
	public int updateProfileImage(String id, imageDetailDTO iddto) {
		int result=0;
		String sql = "update image_detail set img_name=? where img_no=(select img_no from image i where section_no=?)";
	...
	public int deleteProfileImage(int pmno) {
		...
		String sql ="delete from image where section_no=(select pm_id from purchase_member p where pm_no=?) and section='I04'";
	...
	public int deleteReviewImage(String pmid) {
		int result=0;
		String sql="delete from image where section_no in(select rw_no from purchase_review pr where pm_id=?) and section='I03'";
