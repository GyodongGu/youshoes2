...
/**
 * @author 유승우 상위 DAO 클래스를 상속받아서 bookmarkDAO를 생성 1. 즐겨찾기한 목록 갖고오기 bookmarkGet()
 * 
 */
...
public class bookmarkDAO extends DAO {
	...
	public List<bookmarkDTO> bookmarkGet(int id) { // 1. 즐겨찾기한 목록 갖고오기 bookmarkGet()
		...
		String sql = "select p.*, getimage('I02', p.pdt_no) as img_name " + "from bookmark bk join product p " + "on bk.pdt_no = p.pdt_no " + "where pm_no=?"; // 접속한 유저의 찜 목록의 이름 갖고오기
		try {
